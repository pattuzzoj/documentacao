...
<article>
    ...
    if - define uma estrutura SE
    else if - define uma estrutura SENÃO SE
    else - define uma estrutura SENÃO

    switch - define uma estrutura 
    case - define um caso
    default - define um caso padrão

    for - define uma estrutura de repetição PARA
    while - define uma estrutura de repetição ENQUANTO
    do - define a execução de uma instrução antes de sua repetição
...
<article>
    ...
    Para: for (declaração_de_variável, condição, manipulação_de_variável) {
        código
    }

    Para em: for (index in object) {
        código
    }

    Para de: for (valor of object) {
        código
    }
</article>
